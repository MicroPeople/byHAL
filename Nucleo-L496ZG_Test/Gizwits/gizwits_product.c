      ...
      case EVENT_LED_COLOR:
        ...
        switch(currentDataPoint.valueLED_Color)
        {
          ...
          case LED_COLOR_VALUE1:
            //user handle
					{
						setAllPixelColor(174, 0, 37);
					}
            break;
          case LED_COLOR_VALUE2:
            //user handle
					{
						setAllPixelColor(88, 53, 0);
				  }
            break;
          case LED_COLOR_VALUE3:
            //user handle
					{
           setAllPixelColor(42, 214, 50);
				}
            break;
          default:
            break;
...
int32_t uartWrite(uint8_t *buf, uint32_t len)
{
    ...
    for(i=0; i<len; i++)
    {
HAL_UART_Transmit(&huart3, (uint8_t *)&buf[i], 1,HAL_MAX_DELAY); 
#ifdef PROTOCOL_DEBUG
        GIZWITS_LOG("%02x ", buf[i]);
#endif
        if(i >=2 && buf[i] == 0xFF)
        {
HAL_UART_Transmit(&huart3, (uint8_t *)&aa, 1,HAL_MAX_DELAY); 
#ifdef PROTOCOL_DEBUG
